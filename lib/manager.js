...
import { iterateConditional, arrayRemove, arrayFindByKey, arrayRemoveByKey, millisecondsPerFrame, isTypeSeconds } from './utils'
import 'raf-polyfill'
...
export const createTimer = singleBird => {
	...
	if (isTypeSeconds(singleBird.lifeSpanIn)) {
		...
		lifeMilli = millisecondsPerFrame(singleBird._fps) * Math.max(singleBird.lifeSpan - 1, 1)
	}
