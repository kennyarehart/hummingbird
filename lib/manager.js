...
export const enlist = (singleBird, skipFinalCheck, fromWake) => {
	...
	console.log('\t1. singleCadence:', singleCadence)
	if (singleCadence) console.log('\t| pool:', singleCadence.pool.length, '| sleepers:', singleCadence._sleepPool.length)
	// if none found, make a new one
	if (!singleCadence) {
		singleCadence = new Cadence(fps)
		_cadencePool.push(singleCadence)

		console.log('\t2. singleCadence:', singleCadence)
	}
	...
	if (fromWake) {
		console.log('fromWake ~> remove from that sleep pool')
		// move from the single cadence sleep pool to active pool
		singleBird._parent.wakeSingle(singleBird)
		console.log('BOOM:', singleBird._parent.pool.length, singleBird._parent._sleepPool.length)
		return singleBird
	}

	console.log('_isActive:', _isActive, '| _isPaused:', _isPaused)
...
export const releaseSingle = (singleBird, skipFinalCheck) => {
	...
	if (singleCadence) {
		...
		if (singleCadence.pool.length === 0) {
...
function handleNext() {
	...
	// console.log('handleNext(), _isPaused:', _isPaused, '| _isActive:', _isActive, '| _cadences:', _cadencePool)
	if (!_isPaused) {
		for (var i = 0, k = _cadencePool.length; i < k; i++) {
			_cadencePool[i].tick()
		}
