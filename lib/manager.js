...
export const createTimer = singleBird => {
    ...
    const lifeMilli = singleBird.life * 1000
    const singleTimer = new Timeblock(lifeMilli, handleTimer, singleBird)
    // _timers[singleTimer.id] = singleTimer
    _timersArray.push(singleTimer)
    return singleTimer
}
...
function handleNext() {
    ...
    if (!_isPaused) {
        ...
        //     _timers[key].tick()
        // }
        for (var i = 0, k = _timersArray.length; i < k; i++) {
            _timersArray[i].tick()
        }
...
function handleTimer(timer) {
    ...
    for (var i = _timersArray.length - 1, k = 0; i >= k; i++) {
        if (_timersArray[i] === timer) {
            _timersArray.splice(i, 1)
            break
        }
    }
