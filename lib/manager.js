...
export const enlist = (singleBird, skipFinalCheck, fromWake) => {
	...
	// if none found in active pool & if is from a wake(), check if the cadence is in the sleep pool as well
	if (!singleCadence && fromWake) {
		singleCadence = arrayRemoveByKey(_cadenceSleepPool, 'fps', fps)
...
export const releaseSingle = (singleBird, skipFinalCheck, fromSleep) => {
	...
	if (singleCadence) {
		...
		if (singleCadence.pool.length === 0) {
			...
			if (fromSleep) {
				_cadenceSleepPool.push(singleCadence)
			}
		}
...
export const sleepSingle = singleBird => {
	...
	releaseSingle(singleBird, false, true)

	toggleLife(singleBird, true)
}

export const wake = (...args) => {
	console.log(':: wake() :: _cadencePool:', _cadencePool)
